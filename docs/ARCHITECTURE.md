...
Whisk follows Clean Architecture: a pure kernel behind ports, adapters at
...
```
                      Whisk  (executable — composition root, sink)
                         │ may name everything
          ┌──────────────┼──────────────────┐
          ▼              ▼                  ▼
...
1. **One bounded context.** `WhiskKernel` holds `Entities/` and
...
4. **The composition root is a sink.** The `Whisk` executable wires
