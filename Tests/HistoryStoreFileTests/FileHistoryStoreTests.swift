...
@Suite struct FileHistoryStoreContract {
    ...
    @Test func a_saved_history_loads_back_identically() throws {
        ...
        let items = [
            ClipboardItem(id: UUID(), payload: .text("héllo\nworld"), sourceApp: "Safari", copiedAt: date(ms: 1_700_000_000_123), isPinned: true),
            ClipboardItem(id: UUID(), payload: .link(url), sourceApp: nil, copiedAt: date(ms: 1_700_000_001_456), isPinned: false),
            ClipboardItem(id: UUID(), payload: .image(Data([0x89, 0x50, 0x4E, 0x47])), sourceApp: "Preview", copiedAt: date(ms: 1_700_000_002_789), isPinned: false),
            ClipboardItem(id: UUID(), payload: .fileReferences(["/tmp/a.txt", "/tmp/b.txt"]), sourceApp: "Finder", copiedAt: date(ms: 1_700_000_003_000), isPinned: true),
        ]
    ...
    @Test func a_blob_no_longer_referenced_is_removed_on_save() throws {
        let (store, directory) = try makeStore()
        defer { try? FileManager.default.removeItem(at: directory) }
        let image = ClipboardItem(id: UUID(), payload: .image(Data([0x01])), sourceApp: nil, copiedAt: date(ms: 1_700_000_000_000), isPinned: false)
        let text = ClipboardItem(id: UUID(), payload: .text("survivor"), sourceApp: nil, copiedAt: date(ms: 1_700_000_001_000), isPinned: false)
    ...
    @Test func a_stored_entry_with_an_unknown_kind_is_skipped_not_fatal() throws {
        ...
        defer { try? FileManager.default.removeItem(at: directory) }
        try store.save([
            ClipboardItem(id: UUID(), payload: .text("valid"), sourceApp: nil, copiedAt: date(ms: 1_700_000_000_000), isPinned: false)
        ])
